...
%>
          ...
          <tr> 
            <td>常用设置链接：[<a href="?Action=jsqquickset">计数器基本样式快速设置</a>]&nbsp;[<a href="?Action=jsqset">计数器参数设置</a>]&nbsp;[<a href="?Action=stylelist">计数器图片样式选择</a>]&nbsp;[<a href="?Action=getcode">预览及获得计数器代码</a>]&nbsp;[<a href="?Action=modifypassword">修改密码</a>]&nbsp;</td>
          </tr>
        </table>
        <table class="tb_1">
          <tr class="tr_1">
          ...
          <tr > 
            <td >站点地址：</td>
            <td colspan="3" ><a href="<%=RsUser("PageUrl")%>" target="_blank"><%=RsUser("PageUrl")%></a>&nbsp;[<a href="http://www.alexa.com/data/details/traffic_details?q=&url=<%=RsUser("PageUrl")%>" target="_blank">Alexa排名查询</a>]&nbsp;[<a href="http://web.archive.org/web/*/<%=RsUser("PageUrl")%>" target="_blank">历史网页回忆</a>]</td>
          </tr>
          <tr > 
            <td >开始统计于：</td>
...
%>
            <td ><%=Rs("MyCounter")%></td>
            <td ><%=Rs("IpCounter")%></td>
            <td >最高访问量： <%=TopIp%> IP (发生在: <%=TopIpDate%>) </td>
          </tr>
          <%
...
%>
          <tr > 
            ...
            <td ><%=MyCounter%></td>
            <td ><%=IpCounter%></td>
            <td >最高浏览量： <%=TopShow%> PV (发生在: <%=TopShowDate%>)</td>
          </tr>
          <%
...
%>
          <tr > 
            ...
            <td ><%=Rs(0)%></td>
            <td ><%=Rs(1)%></td>
            <td >最低访问量： <%=LowIp%> IP (发生在: <%=LowIpDate%>)</td>
          </tr>
          <tr > 
            <td >平均每日：</td>
            ...
            <td >最低浏览量： <%=LowShow%> Pv (发生在: <%=LowShowDate%>)</td>
          </tr>
          <tr > 
            <td >预计今日：</td>
            <td > 
...
%>
          ...
          <tr class="tr_2"> 
            ...
            <td class="td_nowrap"><a href="?Action=<%=Action%>&AddDate=<%=AddDate%>&Px=LyHead">来源网站</a></td>
            <td class="td_nowrap"><a href="?Action=<%=Action%>&AddDate=<%=AddDate%>&Px=MyCounter">PV数量</a></td>
			<td class="td_nowrap"><a href="?Action=<%=Action%>&AddDate=<%=AddDate%>&Px=IpCounter">IP数量</a></td>
...
<table class="tb_1">
<tr class="tr_1">
            <td colspan="8" >在线人数统计<%If TotalRs>1 Then Response.write "&nbsp;["&TotalTime&"分钟内有"&TotalRs&"个人访问]"%></td>
          </tr>
          <tr class="tr_2"> 
            <td class="td_nowrap">序号</td>
            <td class="td_nowrap">IP</td>
...
%>
          ...
          <tr class="tr_2"> 
            <td class="td_nowrap"><a href="?Action=<%=Action%>&AddDate=<%=AddDate%>&Px=ID">序号</a></td>
            <td class="td_nowrap"><a href="?Action=<%=Action%>&AddDate=<%=AddDate%>&Px=WebUrl">网站页面</a></td>
...
%>
          ...
          <tr> 
            <td colspan="4"> 共<%=ShowTotal%>次浏览，<%=IpTotal%>个IP，平均每月<%=Int(ShowTotal/12)%>次访问，平均每月IP<%=Int(IpTotal/12)%>个</td>
          </tr>
        </table>
  <%end if%>
...
loop%>
          ...
          <tr> 
            <td colspan="4"><%=AddDate%>此日期：总计浏览<%=ShowTotal%>次 IP<%=IpTotal%>个</td>
          </tr>
        </table>
...
%>
  <tr class="tr_1"> 
    <td colspan="5"><%=SiteDesc%>关键字统计[可点击标题排序]<br>
      </td>
  </tr>
  <tr class="tr_2"> 
