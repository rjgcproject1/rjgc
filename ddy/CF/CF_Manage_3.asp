...
d_1.style.display = "none";
 ...
 <tr class="tr_1">
  ...
  <tr > 
    <td colspan="2"  class="td_1">Script调用： 
    </td>
  </tr>
  <tr > 
    <td colspan="2"  class="td_1"><script src="<%=Tmp%>cf.asp?UserName=<%=Rs("UserName")%>"></script></td>
  </tr>
</table>
<%End If%>
        ...
        <table class="tb_1">
          ...
          <tr class="tr_1">
            ...
            <tr > 
              ...
              <td   class="td_1"><table border='0' cellpadding='0' cellspacing='0'>
                  <tr> 
			...
			<tr  id="a_1">
              ...
              <td   class="td_1"><select name="tjopen">
            ...
            <tr > 
              ...
              <td   class="td_1"> 
                ...
                <br>
                (注：这里只提供四种基本样式选择,更多的设置请进入＂<a href="?Action=jsqset">参数设置</a>&quot;和&quot;<a href="?Action=stylelist">图片样式选择</a>&quot;，你的设置计数器实际效果在&quot;<a href="?Action=getcode">预览及获得代码</a>&quot;里可以看到)</td>
            </tr>
          </form>
        </table>
<%End If%>      
...
<%end if%>
        ...
        <table class="tb_1">
          ...
          <tr class="tr_1">
              <td colspan="2"> 将根据以下你的设置重置你的计数器，请慎重操作！</td>
            </tr>
            <tr > 
...
<form name="form3" method="post" action="?Action=userdelsave">
          <tr class="tr_1">
              <td colspan="2"> 账号删除，你确定是否真的要删除！</td>
    </tr>
            <tr > 
...
Rs.Open Sql,Conn,1,1%>
  ...
  <form name="form3" method="post" action="?Action=pwdmodifysave">
          <tr class="tr_1">
      <td height="21" colspan="2"> 修改密码(带*为必填项）</td>
    </tr>
    <tr > 
...
//-->
        ...
        <table class="tb_1">
          <tr class="tr_1">
    <td class="td_1">获得计数器代码及计数器预览</td>
  </tr>
  <tr > 
    <td  class="td_1">两种代码方式可供选择：<input type="submit" name="Submit5" value="Script网站方式调用" onclick="location.href='?Action=getcode'"<%If Action="getcode" Then%> style="BACKGROUND-COLOR: #e8e8e8;"<%End If%>> 
      <input type="submit" name="Submit6" value="Img网店类方式调用" onclick="location.href='?Action=getcode_2'"<%If Action="getcode_2" Then%> style="BACKGROUND-COLOR: #e8e8e8;"<%End If%>></td>
  </tr>
  <tr > 
    <td  class="td_1">Script网站方式调用，此方式适合放在自己的网站页面上，有几十种图片样式可以选择，请在菜单"<a href="?Action=stylelist" target="_blank">计数器设置>>图片样式选择</a>"里选择!设置计数器的一些参数请在请在菜单"<a href="?Action=jsqset" target="_blank">计数器设置>>参数设置</a>"里设置，打造你满意计数器!</td>
  </tr>
  <tr > 
    <td  class="td_1">以下方框内是你的计数器代码,script调用方式，请复制后插入到网页源代码里:<br> 
      ...
      <br>
      你的设置的计数器script方式调用预览： <br> <script src="<%=Tmp%>cf.asp?username=<%=UserName%>"></script> 
    </td>
  </tr>
</table>
<%end if%>
...
//-->
  ...
  <tr > 
    <td  class="td_1">Img网店类方式调用，此方式适合放在不是自己网站的网页上，Img网店类方式计数器可以在任何能插入图片的地方使用,不过此调用方式的计数器功能少一些，只有一种固定图片样式！</td>
  </tr>
  <tr > 
    <td  class="td_1">以下方框内是你的计数器代码，Img调用方式，请复制后插入到网页源代码里,这种方式调用你可以把你的计数器看做是一个图片，图片地址是<a href="<%=Tmp%>cf.asp?c=<%=UserName%>" target="_blank"><%=Tmp%>cf.asp?c=<%=Rs("UserName")%></a>你甚至可以把此地址当作图片论坛发贴时插入，一样记数的,设置隐藏时显示数为0！<br> <textarea name="countercode2" cols="50" rows="6" id="textarea"><a href="<%=Tmp%>Index.asp" target="_blank"><img src="<%=Tmp%>cf.asp?c=<%=UserName%>" border="0" title="<%=server.HTMLEncode(GetUnicode(RsSet("SysTitle")))%>"></a></textarea> 
