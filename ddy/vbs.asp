...
StrTmp = "<ul>"''<table width=""100%"" border=""0"" cellspacing=""0"" cellpadding=""0"">
Do While Not Rs.Eof
StrTmp = StrTmp & "<li>"''<tr>
For II=1 To Cels 
If Rs.Eof Then
	StrTmp = StrTmp & "" ''"<td>&nbsp;</td>"
Else
	''add by jieqiuming 2012-6-01
	If OrderType <> "new" Then
		StrTmp = StrTmp & "<img src=""img/3_19.jpg""/>"''<td class=""newstitle""><div>
	End If
	If DisClass Then
...
Response.Write "function " & RndFun & "()"
Response.Write "{"
	Sql="select top 10 Id,Title,PicFile,TitleColor,TitleURL from Ok3w_Article where ChannelID=1 and IsPass=1 and IsPlay=1 and IsDelete=0"
	If ClassID<>"" Then Sql=Sql & " and SortPath like '%," & ClassID & ",%'"
	Sql=Sql & " order by IsTop desc,IsCommend desc,Id desc"
	Rs.Open Sql,Conn,0,1
	pics = ""
...
Response.Write("<div>")
Response.Write("<div>" & oRs("SortName") & "<span><a href=""" & "list.asp?id=" & oRs("ID") & """>更多>></a></span></div>")
Response.Write("<div>")
...
Response.Write("<div class=""job""><ul>")''<table width=""100%"" border=""0"" align=""center"" cellpadding=""0"" cellspacing=""0"">
...
i_Count = i_Count + 1
Response.Write("  <li>")''<tr>
Response.Write("    <img src=""img/3_19.jpg"">" & Format_TitleURL("show.asp",Rs("ID"),Rs("Title"),Rs("TitleColor"),Rs("TitleURL"),50) & "</td>")
Response.Write("    <span class=""date"">" & Format_Time(Rs("AddTime"),1) & "</span>")
Response.Write("  </li>")
Rs.MoveNext
Page.MoveNext
''If i_Count Mod 5 = 0 Or Rs.Eof Then
''Response.Write("  <li>")''<tr>
''Response.Write("    <td colspan=""2""><div></div></td>")
''Response.Write("  </li>")''</tr>
''End If
Loop
Rs.Close
...
Else
Response.Write("			<tr>")
Response.Write("              <td><br /><br /><h1>Sorry!没业魏荨</h1><br /><br /></td>")
Response.Write("            </tr>")
End If
Rs.Close
Response.Write("</table>")
...
Private Sub Ok3w_Article_SmallClass(ClassID)	
	...
	End If
	If Not Rs.Eof Then
	Response.Write("<div class=""title3"">")
	Response.Write("<strong style=""font-size:14px;color: #FFFFFF;line-height: 34px;margin-left:18px"">" & GetParentClassName(ClassID) & "</strong><br>")
	Do While Not Rs.Eof
		If Rs("gotoURL")="" Then
			Response.Write("<span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span><a href=""" & "list.asp?id=" & Rs("ID") & """>" & Rs("SortName") & "</a><img src=""images/nyleftxuxianbg.gif""/><br>")
		Else
			Response.Write("<a href=""" & Rs("gotoURL") & """ target=""_blank"">" & Rs("SortName") & "</a><br>")
		End If
		Rs.MoveNext
...
Private Sub Ok3w_Article_Class_NavLists(ClassID)
Sql="select ID,SortName from Ok3w_Class where ParentID in(" & ClassID & ") order by ID"
Set oRs = Conn.Execute(Sql)
Response.Write("<div class=""leftBarBody"">")
Do While Not oRs.Eof
	Response.Write("<div class=""leftBarItem"">")
	Response.Write("<a href=""" & "list.asp?id=" & oRs("ID") & """>&gt;&gt; " & oRs("SortName") & "</a>")
	Response.Write("</div>")
	oRs.MoveNext
	'If Not oRs.Eof Then Response.Write(" &gt;&gt; ")
...
''add by jieqiuming on 2012-7-3
Private Sub Ok3w_Article_Class_NavMenus()
Sql="select ID,SortName from Ok3w_Class where ChannelID=1 and ParentID=0 and IsNav=1 order by ID"
Set oRs = Conn.Execute(Sql)
Response.Write("<ul id=""jsddm"">")
Response.Write("<li ><a href=""index.asp"" style=""width:70px"">首页</a></li>")
Do While Not oRs.Eof
	Response.Write("<li>")
	Response.Write("<a href=""" & "list.asp?id=" & oRs("ID") & """>" & oRs("SortName"))' & "</a>"
	
	Sql2="select ID,SortName from Ok3w_Class where ChannelID=1 and IsNav=1 and ParentID="& oRs("ID") &" order by ID"
	Set oRs2 = Conn.Execute(Sql2)
	
	if Not oRs2.Eof Then  Response.Write("<img src=""img/arrow.gif"">")
	...
	Response.Write("<li>")
	Response.Write("<a href=""" & "list.asp?id=" & oRs2("ID") & """>" & oRs2("SortName") & "</a>")
	oRs2.MoveNext
	Response.Write("</li>")
	Loop
	Response.Write("</ul>")
...
If Not Rs.Eof Then
StrTmp = StrTmp & "	<a href=""" & Rs("Lurl") & """ target=""_blank""><img src=""" & Rs("Lpic") & """ alt=""" & Rs("Lname") & """ width=""88"" height=""31"" border=""0"" style=""border:1px solid #CCCCCC; padding:1px;"" /></a><br>"
	Rs.MoveNext
Else
StrTmp = StrTmp & "	&nbsp;"
End If
...
StrTmp = StrTmp & "    <td align=""center"" width=""" & 100\LeftN & "%"">"
If Not Rs.Eof Then
StrTmp = StrTmp & "	<a href=""" & Rs("Lurl") & """ target=""_blank"">" & Rs("Lname") & "</a><br>"
Rs.MoveNext
Else
StrTmp = StrTmp & "	&nbsp;"
End If
